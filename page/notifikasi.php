...
$queryNotifikasi = "SELECT * FROM user JOIN user_notifikasi ON user.id = user_notifikasi.user_id WHERE user.id = " . $myUser['id'] . " ORDER BY waktu_notifikasi DESC";
$notifications = query($queryNotifikasi);

// Merubah Semua Notifikasi
$queryReadNotifikasi = "UPDATE user_notifikasi SET sudah_dilihat = 1 WHERE user_id = " . $myUser['id'];
mysqli_query($db, $queryReadNotifikasi);
...
<html lang="en">
	<head>
		...
		<link
			...
			type="image/x-icon"
		/>
		<title>Meridio | Notifikasi</title>
	</head>
	<body>
		<!-- Navigation Bar -->
					...
					<div class="col-md-12">
						<a href="./notifikasi.php">
                            <svg
                                    ...
                                    height="2em"
                                    viewBox="0 0 16 16"
                                    class="bi bi-bell d-inline mb-3 mr-1"
                                    fill="currentColor"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path d="M8 16a2 2 0 0 0 2-2H6a2 2 0 0 0 2 2z" />
							...
							<h1 class="d-inline">Notifikasi</h1>
						</a>
					</div>
				</div>
				<div class="row mt-4 d-flex justify-content-center">
					<div class="col-md-8 mb-4">
						<?php if (count($notifications) <= 0): ?>
							<h1 class="text-center">Notifikasi masih kosong</h1>
						<?php else: ?>
							<?php foreach ($notifications as $notification): ?>
								<div class="notifikasi-<?=$notification['id']?> bg-notifikasi p-3 rounded-lg mt-3">
                                    <?php
                                        $time = strtotime($notification['waktu_notifikasi']);
                                        $time = date('d M Y • H:i', $time);
                                    ?>
                                    <span class="d-block"><?=$time?> WIB</span>
                                    <span><?=$notification['isi_notifikasi']?></span>
                                </div>
							<?php endforeach?>
						<?php endif?>
					</div>
