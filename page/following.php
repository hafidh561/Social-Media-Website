...
$queryFollowings = "SELECT * FROM user JOIN user_following ON user.id = user_following.user_id WHERE user.id = " . $_GET['id'] . " ORDER BY waktu_follow DESC";
$followings = query($queryFollowings);

$semuaFollowings = [];
$waktuFollowings = [];
foreach ($followings as $following) {
    $semuaFollowings[] = $following['user_id_following'];
    $waktuFollowings[] = $following['waktu_follow'];
}
	...
	<body>
		...
		<section class="bagian-tengah pb-4">
			<div class="container pt-5 mt-5">
				<div class="row">
					<div class="col-md-12">
						<a href="./following.php?id=<?=$_GET['id']?>">
							<svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" fill="currentColor" class="bi bi-people mb-3 mr-1" viewBox="0 0 16 16">
							...
							<h1 class="d-inline">Following</h1>
						</a>
					</div>
				</div>
				<div class="row mt-5 d-flex justify-content-center">
					<div class="col-md-8 mb-4">
						<?php if (count($followings) <= 0): ?>
							<h1 class="text-center">Following masih kosong</h1>
						<?php else: ?>
							<?php for ($i = 0; $i < count($semuaFollowings); $i++): ?>
								<?php $following = query("SELECT * FROM user WHERE id = $semuaFollowings[$i]")?>
								<?php foreach ($following as $follow): ?>
									<div class="following-<?=$follow['id']?> pt-3 pr-3 pl-3 rounded-lg">
										<div class="account-following-<?=$follow['id']?>">
											...
											<div class="row">
												<div class="col">
													<a href="./profile.php?id=<?=$follow['id']?>" target="_blank">
														<span class="font-weight-bold"><?=$follow['username']?>
														</span>
													</a>
													<?php if (isset($_SESSION['login'])): ?>
														<?php
											...
											<div class="row">
												<div class="col">
													<?php
														$time = strtotime($waktuFollowings[$i]);
