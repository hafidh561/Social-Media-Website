...
$('.savedArticle, .unsavedArticle').on('click', function () {
	...
	if (!$post.hasClass('unsavedArticle')) {
		$.ajax({
			url: './page/ajax/unsaved article.php',
			type: 'post',
			data: {
				unsaved: 1,
				postid: postid,
		...
		$.ajax({
			url: './page/ajax/saved article.php',
			type: 'post',
			data: {
				saved: 1,
				postid: postid,
